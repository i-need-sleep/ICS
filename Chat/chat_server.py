...
# ==============================================================================
                ...
                message = "(" + from_name + ")" + ctime + " " + message
            ...
            #File Transfer!! (ummmm... it's basicly the same thing as regular messages)
                ...
                #Send a conformation message back to the sender
                if msg["pack"] == "END":
                    mysend(from_sock, json.dumps(
                        {"results": "\nFile Sent!"}))
                    
            #Leaderboard!!
            elif msg["action"] == "submit_highscore":
                score = msg["score"]
                player = msg["player"]
                if player in self.leaderboard.keys():
                    if score >= self.leaderboard[player]:
                        self.leaderboard[player] = score
                        Out = "Score submitted: "+str(score)
                    else:
                        Out = "A higher score already exists!"
                else:
                    self.leaderboard[player] = score
                    Out = "Score submitted: "+str(score)
                mysend(from_sock, json.dumps({"results":Out}))
            
            elif msg["action"] == "view_leaderboard":
                mysend(from_sock, json.dumps({"results":self.leaderboard}))
...
# ==============================================================================
                ...
                poem = "".join(self.sonnet.get_poem(poem_num))
...
# ==============================================================================
                ...
                for i in search_rslt:
                    out.append(i[1])
                print('server side search: ' + str(out))

                # ---- end of your code --- #
                mysend(from_sock, json.dumps(
                    {"action": "search", "results": str(out)}))
