...
class ClientSM:
    ...
    def SendFile(self):
        ...
        FO.close()
        #breaking down the file into seperate messages
        file_length = len(FileMsg)
        file_name = self.file_dir.split("/")[-1]
        self.out_msg += "[" + self.me + "]" + ": Sending File: " + self.file_dir
        for i in range(1,file_length//50000+2):
            current_pack = FileMsg[(i-1)*50000:i*50000]
            if i == file_length//50000+1:
                i = "END"
            msg = "Sent file: " + file_name
            Out = {"action":"file", "from":"[" + self.me + "]", "message":msg,"file":current_pack,"file_name":file_name,"pack":i}
...
#==============================================================================
            ...
            if len(peer_msg) > 0:    # peer's stuff, coming in
                ...
                #Receving file!!
                    ...
                    from_name = str(peer_msg["from"])
                    self.file_temp += peer_msg['file']
                    if peer_msg["pack"] == "END":
                        FO = open(self.file_receive+"/"+peer_msg["file_name"],"wb")
                        bin_data = bytes(self.file_temp,codec)
