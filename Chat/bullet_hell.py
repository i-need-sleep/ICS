...
def update_highscore(score):
    if cmdl.client.sm.high_score < score:
        cmdl.client.sm.high_score = score
...
#setting up the difficulty features including how fast the enemies generate and how many scores can the player get                
            ...
            if time1 > 3999 and game_over == False and len(x_bullet) < 1 and len(x_bullet_type_2) < 1 and len(x_bullet_type_3) < 1 and len(x_yellow_enemie) < 1 and len(x_green_enemie) < 1 and len(x_red_enemie) < 1 and len(x_blue_enemie) < 1:
...
#                
        ...
        if game_over == True:
            ...
            update_highscore(int(time_survive))
